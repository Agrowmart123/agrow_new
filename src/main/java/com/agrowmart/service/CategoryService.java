...
//    }
 ...
 // CREATE Category
    ...
    public CategoryResponseDTO create(CategoryCreateDTO dto, User currentUser) {
        if (currentUser == null) {
            throw new ForbiddenException("You must be logged in to create a category");
        }
        ...
        // Auto-assign root parent for restricted vendors if not provided
        if (requiredRootSlug != null && parentIdToUse == null) {
            Category root = categoryRepo.findBySlug(requiredRootSlug)
                    .orElseThrow(() -> new ResourceNotFoundException(
                            "System root category '" + requiredRootSlug + "' not found. Contact admin."
                    ));
            parentIdToUse = root.getId();
        }

        // Parent is mandatory now
        if (parentIdToUse == null) {
            throw new BusinessValidationException("Parent category is required. Root categories cannot be created.");
        }

        Category parent = categoryRepo.findById(parentIdToUse)
                .orElseThrow(() -> new ResourceNotFoundException("Parent category not found"));

        // Restricted vendors: enforce hierarchy
        if (requiredRootSlug != null && !isUnderRoot(parent, requiredRootSlug)) {
            throw new ForbiddenException(
                    "You can only create categories under your allowed section ('" + requiredRootSlug + "')."
            );
        }
...
//    }
 ...
 // UPDATE Category
    ...
    public CategoryResponseDTO update(Long id, CategoryCreateDTO dto, User user) {
        if (user == null) {
            throw new ForbiddenException("You must be logged in to update categories");
        }
        ...
        // Restricted vendors can only update in their section
        if (allowedRoot != null && !isUnderRoot(cat, allowedRoot)) {
            throw new ForbiddenException("You can only update categories in your section");
        }
        ...
        if (dto.name() != null && !dto.name().trim().isEmpty()) {
            cat.setName(dto.name().trim());
        }
        if (dto.slug() != null && !dto.slug().trim().isEmpty()) {
            cat.setSlug(dto.slug().trim());
        }

        // Parent change (optional)
        if (dto.parentId() != null) {
            Category newParent = categoryRepo.findById(dto.parentId())
                    .orElseThrow(() -> new ResourceNotFoundException("New parent category not found"));

            if (allowedRoot != null && !isUnderRoot(newParent, allowedRoot)) {
                throw new ForbiddenException("Cannot move category outside your designated section.");
            }

            cat.setParent(newParent);
        } else if (dto.parentId() == null && cat.getParent() != null) {
            // Trying to detach → make root
            if (allowedRoot != null) {
                throw new ForbiddenException("Restricted vendors cannot detach subcategories to make them root-level.");
            }
            cat.setParent(null);
        }
...
//    }
 ...
 // ──────────────────────────────────────────────
    ...
    public void delete(Long id, User user) {
        if (user == null) {
            throw new ForbiddenException("You must be logged in to delete categories");
        }
        ...
        if (allowedRoot != null && !isUnderRoot(cat, allowedRoot)) {
            throw new ForbiddenException("You can only delete categories in your section");
        }
...
//    }
 ...
 // ──────────────────────────────────────────────
    // GET Allowed Categories for Current Vendor
    // ──────────────────────────────────────────────
    public List<CategoryResponseDTO> getAllowedCategoriesForVendor(User currentUser) {
        if (currentUser == null) {
            throw new ForbiddenException("You must be logged in to view allowed categories");
        }
        ...
        // Restricted vendors → only under their root
        Category root = categoryRepo.findBySlug(requiredRootSlug)
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Your section root category not found. Contact support."
                ));

        List<Category> allowed = new ArrayList<>();
        collectSubcategories(root, allowed);

        return allowed.stream().map(this::toDto).toList();
    }
